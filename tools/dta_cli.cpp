...
struct ContentTableEntry {
    ...
    uint32_t OffsetToFileInfoHeader;
...
enum BlockType {
    ...
    BLOCK_LZSS_RLE = 1,
    BLOCK_DPCM0 = 8, BLOCK_DPCM1 = 12, BLOCK_DPCM2 = 16,
    BLOCK_DPCM3 = 20, BLOCK_DPCM4 = 24, BLOCK_DPCM5 = 28, BLOCK_DPCM6 = 32
};

struct DtaInfo { uint32_t k1; uint32_t k2; const char* description; };

// WAV_DELTAS table (verbatim from upstream)
...
static const DtaInfo DEFAULT_DTA_INFO = { 0xA1B2C3D4, 0x23463458, "Unknown DTA (Defaults)" };

static std::map<std::string, DtaInfo> DTA_MAP = {
...
static void DecryptBlock(void* pBuffer, size_t size, uint32_t k1, uint32_t k2) {
...
static void DecompressLZSS(const std::vector<char>& input, std::vector<char>& output) {
    size_t pos = 0;
...
static void DecompressDPCM(int dpcmType, uint8_t* buffer, size_t bufferLen,
                           std::vector<char>& out, WavHeader& wavHeader,
    ...
    if (wavHeader.channels == 1) {
        ...
        while (pos < bufferLen) { out.push_back(buffer[pos]); pos++; }
    }
}

static void CreatePathRecursively(const std::string& path) {
    if (path.empty()) return;
    char buffer[MAX_PATH];
    strncpy(buffer, path.c_str(), MAX_PATH - 1);
    buffer[MAX_PATH - 1] = 0;
...
// Return last path component (filename) from a path string.
...
// Returns number of files extracted, or -1 on hard failure.
static int ExtractDta(const std::string& dtaPath, const std::string& outDir, bool quiet) {
    std::string fname = BaseName(dtaPath);
    std::string upperName = ToUpper(fname);

    uint32_t k1 = DEFAULT_DTA_INFO.k1, k2 = DEFAULT_DTA_INFO.k2;
    auto itMap = DTA_MAP.end();
    for (auto it = DTA_MAP.begin(); it != DTA_MAP.end(); ++it) {
        if (ToUpper(it->first) == upperName) { itMap = it; break; }
    }
    if (itMap != DTA_MAP.end()) { k1 = itMap->second.k1; k2 = itMap->second.k2; }

    std::ifstream file(dtaPath, std::ios::binary | std::ios::ate);
    if (!file.is_open()) {
    ...
    if (header.FileCount > 100000 || header.OffsetToContentTable > (uint32_t)fileSize) {
        fprintf(stderr, "ERROR: decrypt failed (bad keys) for %s\n", dtaPath.c_str());
        return -1;
    }
    ...
    // Ensure outDir exists. We chdir into it for relative file writes.
    if (!outDir.empty()) {
        CreatePathRecursively(outDir + "\\");
        if (!SetCurrentDirectoryA(outDir.c_str())) {
    ...
    for (uint32_t f = 0; f < header.FileCount; ++f) {
        ...
        for (uint32_t b = 0; b < fh.BlockCount; ++b) {
            ...
            if (blockType == BLOCK_UNCOMPRESSED) {
                ...
                if (!wavHeaderRead && finalData.size() >= sizeof(WavHeader)) {
                    ...
                    if (memcmp(wavHeader.riff, "RIFF", 4) == 0) wavHeaderRead = true;
                }
            } else if (blockType == BLOCK_LZSS_RLE) {
                std::vector<char> tmp;
                DecompressLZSS(std::vector<char>(blk.begin() + 1, blk.end()), tmp);
                ...
                if (!wavHeaderRead) {
                    if (blk.size() > 1 + sizeof(WavHeader)) {
                        ...
                        DecompressDPCM(dpcmType,
                                       (uint8_t*)blk.data() + 1 + sizeof(WavHeader),
                                       blk.size() - 1 - sizeof(WavHeader),
                                       finalData, wavHeader, k1, k2);
                    }
                } else {
                    DecompressDPCM(dpcmType, (uint8_t*)blk.data() + 1,
                                   blk.size() - 1, finalData, wavHeader, k1, k2);
                }
            } else {
                if (blk.size() > 1) {
                    finalData.insert(finalData.end(), blk.begin() + 1, blk.end());
        ...
        if (out.is_open()) {
            ...
            extracted++;
            if (!quiet) printf("  %s (%u bytes)\n", internalName.c_str(), (unsigned)finalData.size());
        }
    }
    return extracted;
}
