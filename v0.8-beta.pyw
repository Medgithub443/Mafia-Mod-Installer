...
T = {
    "ru": {
        ...
        "open_mod_folder": "Открыть папку мода",
    ...
    "cz": {
        ...
        "open_mod_folder": "Otevřít složku modu",
...
README_KEYWORDS = [
    ...
    "readme", "read me", "readthis", "instruction", "instructions", "guide", "manual", "setup", "install", "notes",
    # Русский/Россия/Украина/Беларусь
    "прочитай", "читай", "инструкция", "руководство", "установка", "заметки", "прочти", "readme_ru", "instruktsiya",
    # Чешский/Чехия/Словакия
    "navod", "pokyny", "prirucka", "instalace", "readme_cs", "readme_cz", "návod", "pokyny", "příručka",
    # Польский/Польша
    "przeczytaj", "instrukcja", "podrecznik", "instalacja", "readme_pl", "czytaj",
    # Немецкий/Германия
    "liesmich", "anleitung", "handbuch", "installation", "readme_de", "hinweise",
    # Украинский/Украина
    "прочитай", "інструкція", "керівництво", "установка", "readme_ua",
    # Итальянский/Италия
    "leggimi", "istruzioni", "manuale", "installazione", "readme_it",
    # Венгерский/Венгрия
    "olvassel", "utasitas", "kezikonyv", "telepites", "readme_hu",
    # Румынский/Румыния
    "citeste", "instructiuni", "manual", "instalare", "readme_ro",
    # Сербский/Сербия
    "procitaj", "uputstvo", "prirucnik", "instalacija", "readme_sr",
    # Турецкий/Турция
    "oku", "talimat", "kilavuz", "kurulum", "readme_tr",
    # Болгарский/Болгария
    "procheti", "instruktsiya", "rukovodstvo", "instalatsiya", "readme_bg"
]

README_EXTS = (".txt", ".pdf", ".md", ".rtf", ".doc", ".docx", ".html", ".htm")

def is_readme_file(file_name, dir_path):
    lower_name = file_name.lower()
    
    # Проверка по ключевым словам
    if any(kw in lower_name for kw in README_KEYWORDS) and lower_name.endswith(README_EXTS):
        return True
    
    # Косвенный признак: единственный текстовый файл в директории
...
def get_path_for_ico(relative_path):
    try:
        # Путь для Nuitka (распакованные файлы во временной папке)
        base_path = os.path.dirname(sys.executable)
        if not os.path.exists(os.path.join(base_path, relative_path)):
             # Путь для PyInstaller или обычного запуска
...
class App(TkinterDnD.Tk if DND_AVAILABLE else tk.Tk):
    ...
    def __init__(self):
        ...
        self.title("Mafia Mod Installer v0.8-beta")
        self.geometry("1000x650")
        try:
            icon_path = get_path_for_ico("mmi.ico")
            if os.path.exists(icon_path):
                self.iconbitmap(icon_path)
        except:
            pass  # Игнорируем, если иконка отсутствует
    ...
    def build_install_tab(self):
        ...
        ttk.Button(btn_frame, text=tr("run_game"), command=self.run_game, width=18).pack(side="left", padx=5)
    ...
    def build_mods_tab(self):
        ...
        ttk.Button(btn_frame, text=tr("open_readme"), command=self.open_readme_file).pack(side="left", padx=5)
    ...
    def build_about_tab(self):
        ...
        self.about_text_widget = tk.Text(about_frame, wrap=tk.WORD, height=15, width=60, font=("Arial", 10))
        self.about_text_widget.pack(padx=30, pady=20, fill="both", expand=True)
        self.about_text_widget.insert("1.0", about_text)
        self.about_text_widget.config(state="normal")  # Делаем текст копируемым
        
        # Контекстное меню для about_text
        self.about_text_widget.bind("<Button-3>", lambda e: self.show_context_menu(e, self.about_text_widget))
    ...
    def show_context_menu(self, event, widget):
        ...
        menu.add_command(label=tr("copy"), command=lambda: self.copy_text(widget))
        menu.add_command(label=tr("select_all"), command=lambda: self.select_all_text(widget))
        try:
    ...
    def show_mod_context_menu(self, event):
        """Показывает контекстное меню для модов"""
        self.mods_table.selection_set(self.mods_table.identify_row(event.y))
        mod = self.get_selected_mod()
        if mod:
            menu = tk.Menu(self, tearoff=0)
            menu.add_command(label=tr("open_mod_folder"), command=lambda: open_path(mod["readmes_dir"]))
            menu.add_command(label=tr("open_backup"), command=lambda: open_path(mod["backup"]))
    ...
    def run_game(self):
        """Запускает Game.exe в выбранной папке игры"""
        game_path = self.current_game.get()
        exe_path = os.path.join(game_path, "Game.exe")
        if os.path.exists(exe_path):
            ...
            messagebox.showerror(tr("error"), "Game.exe не найден в папке игры")
    ...
    def refresh_mods_list(self):
        ...
        for mod in self.mods_data:
            ...
            readme_names = ", ".join([os.path.relpath(os.path.join(root, file), mod_dir) 
                                      ...
                                      for file in files if is_readme_file(file, root)])
    ...
    def open_readme_file(self, event=None):
        """Открывает первый readme файл"""
        mod = self.get_selected_mod()
        if mod and mod.get("readmes_dir"):
            mod_dir = mod["readmes_dir"]
            for root, _, files in os.walk(mod_dir):
                for file in files:
                    if is_readme_file(file, root):
                        readme_path = os.path.join(root, file)
                        try:
                            os.startfile(readme_path)  # Открывает в default программе
                        except Exception as e:
                            messagebox.showerror(tr("error"), str(e))
                        return
